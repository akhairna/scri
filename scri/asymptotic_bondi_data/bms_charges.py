...
def bms_charge_to_vector(charge):
    """Output the ell<=1 modes of a BMS charge as a four-vector."""
    four_vector = np.empty(charge.shape, dtype=float)
    four_vector[..., 0] = charge[..., 0].real
    four_vector[..., 1] = (charge[..., 3] - charge[..., 1]).real / sqrt(6)
    four_vector[..., 2] = (1j * (charge[..., 3] + charge[..., 1])).real / sqrt(6)
...
def bondi_comoving_CoM_charge(self):
    ...
    return bms_charge_to_vector(comoving_CoM_charge)[:, 1:]
